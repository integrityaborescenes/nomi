...
struct Engine {
    ...
    /// для одной и той же фразы encoder бежит 1 раз, не N.
...
impl Engine {
    fn load() -> Result<Self, String> {
        ...
        // Веса FP16 на диске, грузим F32 в RAM — candle::marian::decode() создаёт F32
        // attention mask, конфликтует с F16.
    ...
    fn generate_once(
        ...
        // Source: «phrase </s>». Никаких wc-токенов — модель училась на чистых фразах.
        let encoded = self
            .src_tokenizer
            .encode(phrase.to_string(), false)
            .map_err(|e| format!("tokenize: {e}"))?;
        let mut src_ids: Vec<u32> = encoded.get_ids().to_vec();
        ...
        // На step==0 блокируем ▁<first_word> из previous — для разнообразия retry.
        // Без этого Marian слишком уверена в одном ответе.
        ...
        // No-repeat: блокируем уже эмитированные ▁-токены (начала слов).
        ...
        for step in 0..cfg.max_decode_len {
            ...
            // Если токен — начало слова (▁): запомнить для no-repeat.
...
/// Валидация имени: PascalCase, ≥1 слово, без повторов.
fn is_valid(name: &str) -> bool {
    if !PASCAL_RE.is_match(name) {
        return false;
    }
    let total = count_words(name);
    total >= 1 && unique_word_count(name) == total
}
...
pub fn generate_name(phrase: &str, previous: &[String]) -> Result<NameResult, String> {
    ...
    let pascal = 'outer: {
        for attempt in 0..MAX_ATTEMPTS {
            ...
            let name = engine.generate_once(phrase, temperature, previous)?;
            last = name.clone();
            if is_valid(&name) && !previous.iter().any(|p| p == &name) {
                break 'outer name;
            }
        }
        if is_valid(&last) {
            ...
            return Err(format!(
                "failed to produce valid name after {MAX_ATTEMPTS} attempts; last: {last}"
            ));
        }
    };
